...
@onready var tower = preload("res://scenes/Buildings/smallHouse/smallHouse.tscn")
var currTileLayer0
...
func _on_gui_input(event: InputEvent) -> void:
	if Game.gold >= house.cost:
		...
		if event is InputEventMouseMotion and event.button_mask == MOUSE_BUTTON_LEFT:
			get_child(1).global_position = event.global_position
			var mapPath = get_tree().get_root().get_node("Main/Tilemap/BaseWeed")
			var tile = mapPath.local_to_map(mapPath.to_local(get_global_mouse_position()))
			currTileLayer0 = mapPath.get_cell_atlas_coords(tile)  # 0 -> layer of tilemap
...
func _on_mouse_entered():
	Popups.ItemPopup(Rect2i(Vector2i(global_position), Vector2i(size)), ["Small House","Cost","15","Space","4"])
